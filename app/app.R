...
# =============================================================================
#  CONTOURNEMENT DU BOGUE CHROMIUM 468227
# -----------------------------------------------------------------------------
#  shiny::downloadButton() pose un attribut HTML `download` sur le lien. Dans un
#  navigateur fondé sur Chromium (Chrome, Edge, Brave), cet attribut empêche
#  Shinylive de servir le fichier : le téléchargement échoue, ou produit un
#  fichier .xml inexploitable. Tous les exports de l'application — Word, PDF,
#  carte, graphique — tombent donc ensemble dans la version publiée en ligne,
#  alors qu'ils fonctionnent sous Firefox.
#
#  Retirer l'attribut rétablit le fonctionnement. La manipulation est SANS EFFET
#  en Shiny classique : c'est alors l'en-tête Content-Disposition envoyé par le
#  serveur qui déclenche le téléchargement. Il n'y a donc qu'une seule version
#  du fichier à maintenir, pour le poste de travail comme pour le navigateur.
#
#  Contournement documenté par Posit dans la référence de downloadButton().
# =============================================================================
...
marge_valeurs <- function(etiquettes, noms_axe, largeur_image, taille_axe = 10.5,
  ...
  # La largeur d'un caractère de la valeur en bout de barre est proportionnelle
  # à sa taille de police (référence 3.5) : un texte plus gros réserve une marge
  # plus large, sinon les effectifs des grandes barres seraient coupés.
  besoin  <- max(nchar(as.character(etiquettes))) *
             po_par_car * (taille_val / 3.5) + 0.22
...
#     basculait).
#  3. les couleurs sont choisies pour RESSORTIR sur le fond de carte (OSM, très
#     vert et beige) et pour ne pas empiéter sur le rouge/bleu de la charte,
#     réservés au sexe : indigo pour le public, ocre pour le privé.
secteurs_ref <- sort(unique(data_all$secteur))
# Association explicite par nom : robuste à l'ordre alphabétique et à la
# présence éventuelle d'autres modalités (« Privé sous contrat »...), qui
# reçoivent alors une couleur de réserve distincte.
COUL_SECTEUR <- c("Public" = "#3B4CB8",   # indigo
                  "Privé"  = "#C77A1E")   # ocre
reserve_sect <- c("#5E8C4A", "#8C5A9E", "#4A8C8C", "#B0662E")
col_secteur  <- setNames(
  vapply(seq_along(secteurs_ref), function(i) {
    s <- secteurs_ref[i]
    if (s %in% names(COUL_SECTEUR)) COUL_SECTEUR[[s]]
    else reserve_sect[((i - 1) %% length(reserve_sect)) + 1]
  }, character(1)),
  secteurs_ref)
# Fonction de coloration de la carte, tirée de la MÊME source que les graphiques.
# Simple recherche par nom dans col_secteur : garantit que le cercle, la légende
# Leaflet et les barres ggplot d'un secteur portent EXACTEMENT la même couleur,
# sans dépendre de l'ordre interne de colorFactor.
pal_secteur  <- function(x) {
  couleurs <- col_secteur[as.character(x)]
  couleurs[is.na(couleurs)] <- "#777777"
  unname(couleurs)
}
  ...
  if (opts$circles && nrow(pts) > 0) {
    ...
    map <- map %>%
      addCircleMarkers(data = pts, lng = ~longitude, lat = ~latitude,
                       ...
                       stroke = TRUE, weight = 1.2, opacity = 0.95,
                       fillOpacity = 0.75, popup = ~popup_text) %>%
      # Légende par couleurs explicites (pal_secteur n'est plus un objet
      # colorFactor mais une simple fonction) : chaque secteur avec sa teinte.
      addLegend("bottomright", colors = unname(col_secteur[secteurs_ref]),
                labels = secteurs_ref, title = "Secteur d'établissement",
                opacity = 0.9) %>%
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # Sélection MULTIPLE : plusieurs secteurs et/ou types peuvent être cochés.
      # La liste vide vaut « tous » (aucune restriction) — voir sel_multi().
      selectInput("secteur", "Secteur :",
                  choices  = sort(unique(etab_aggreg$secteur)),
                  multiple = TRUE,
                  selected = character(0)),
      selectInput("categorie", "Type d'établissement :",
                  choices  = sort(unique(etab_aggreg$categorie)),
                  multiple = TRUE,
                  selected = character(0)),
      tags$div(style = "font-size:11px; color:#666; margin:-6px 0 8px;",
               "Laisser vide = tous. Plusieurs choix possibles."),
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("Comparateur",
          tags$div(style = "margin-top:12px;",
            selectInput("comp_etabs", "Établissements à comparer :",
                        choices  = liste_etab_alpha,
                        selected = liste_etablissements[1:min(2, length(liste_etablissements))],
                        multiple = TRUE, width = "100%"),
            tags$div(style = "font-size:11px; color:#666; margin:-6px 0 8px;",
                     "Ajoutez autant d'établissements que souhaité."),
        ...
        tabPanel("Synthèse par commune",
          tags$div(style = "margin:10px 4px 6px;",
            radioButtons("vue_synthese", "Niveau d'agrégation :",
              choices = c("Par commune (effectifs sommés)" = "commune",
                          "Par établissement (détail)"      = "etab"),
              selected = "commune", inline = TRUE)),
          DTOutput("tableau_etab")),
...
server <- function(input, output, session) {
  ...
  # ===========================================================================
  #  FILTRES À SÉLECTION MULTIPLE (secteur, type d'établissement)
  # ---------------------------------------------------------------------------
  #  Une sélection VIDE signifie « aucune restriction » (tous). Ces fonctions
  #  centralisent la logique pour que les ~20 endroits qui filtrent ou titrent
  #  se comportent de façon identique, sans oubli ni divergence.
  # ===========================================================================
  # Restreint un tableau à une sélection multiple ; sélection vide = inchangé.
  appliquer_sel <- function(d, colonne, selection) {
    if (is.null(selection) || length(selection) == 0) return(d)
    d[d[[colonne]] %in% selection, , drop = FALSE]
  }
  sel_secteur   <- reactive(input$secteur)
  sel_categorie <- reactive(input$categorie)
  actif <- function(sel) !is.null(sel) && length(sel) > 0

  # Fragment de sous-titre décrivant une sélection ("Public" ; "2 secteurs :
  # Public, Privé" ; "tous secteurs" si vide). Toujours explicite, pour que
  # l'utilisateur voie sur quoi porte le graphique.
  txt_sel <- function(sel, singulier, pluriel, si_vide) {
    if (!actif(sel)) return(si_vide)
    if (length(sel) == 1) return(sel)
    paste0(length(sel), " ", pluriel, " : ", paste(sel, collapse = ", "))
  }
  txt_secteur   <- function() txt_sel(sel_secteur(),   "secteur", "secteurs",
                                      "tous secteurs")
  txt_categorie <- function() txt_sel(sel_categorie(), "type", "types",
                                      "toutes catégories d'établissement")

  # Un graphique qui analyse une dimension ignore d'ordinaire le filtre sur
  # cette même dimension (sinon il s'auto-annulerait). Mais lorsqu'une sélection
  # multiple est active, l'utilisateur s'attend à ce que le graphique s'y
  # restreigne. On applique donc la restriction ET on l'affiche de façon très
  # visible, en rouge, pour qu'il n'y ait aucune ambiguïté sur le périmètre.
  # `couleur_dim` distingue la restriction (rouge) de l'absence de filtre.
  bandeau_restriction <- function(sel, dimension) {
    ...
    annotate("label", x = Inf, y = Inf, hjust = 1.02, vjust = 1.4,
             ...
             size = 3.5, label.size = 0, label.r = unit(0.12, "lines"),
             label.padding = unit(0.35, "lines"))
  }

  data_filtree <- reactive({
    d <- filter(etab_aggreg, rentree == input$rentree)
    d <- appliquer_sel(d, "secteur",   sel_secteur())
    d <- appliquer_sel(d, "categorie", sel_categorie())
  ...
  sous_titre <- reactive(paste0("Rentrée ", input$rentree,
    if (actif(sel_secteur()))   paste0(" — ", txt_secteur())   else "",
    if (actif(sel_categorie())) paste0(" — ", txt_categorie()) else ""))
  ...
  p_secteur <- reactive({
    # Analyse le secteur : restreint à la sélection si elle existe (voir bandeau).
    ...
    ggplot(d, aes(reorder(secteur, eff), eff, fill = secteur)) +
      ...
      bandeau_restriction(sel_secteur(), "secteur") +
      labs(title = "Répartition par secteur",
           subtitle = paste0("Rentrée ", input$rentree, " — ", txt_categorie()),
           x = NULL, y = "Étudiants inscrits") +
      theme_obs
  })
  ...
  p_cat <- reactive({
    # Ce graphique analyse la catégorie. Par défaut il ignore le filtre sur
    # cette dimension ; mais si l'utilisateur a coché des types précis, il se
    # restreint à eux (et l'affiche en rouge, voir bandeau_restriction).
    ...
    ggplot(d, aes(reorder(categorie, eff), eff)) +
      ...
      coord_flip() +
      scale_y_continuous(labels = fmt_eff, expand = expansion(mult = c(0, 0.22))) +
      bandeau_restriction(sel_categorie(), "type") +
      labs(title = "Répartition par catégorie d'établissement",
           subtitle = paste0("Rentrée ", input$rentree, " — ", txt_secteur()),
           x = NULL, y = "Étudiants inscrits") +
      theme_obs
  })
  ...
  p_evol_secteur <- reactive({
    base <- appliquer_sel(data_multi_secteur(), "secteur", sel_secteur()) %>%
      filtre_periode(input$per_evol_secteur)
    par_secteur <- base %>%
    ...
    d <- bind_rows(total, par_secteur) %>%
      ...
      ajouter_rang()
    st <- paste0(txt_periode(input$per_evol_secteur), " — ", txt_categorie())
    ggplot(d, aes(rentree, eff, colour = `Série`)) +
      geom_line(linewidth = 1.2) +
      geom_point(size = 2.8) +
      couche_etiquettes(4.6) +
  ...
  p_evol_cat <- reactive({
    d <- appliquer_sel(data_multi_cat(), "categorie", sel_categorie()) %>%
      filtre_periode(input$per_evol_cat) %>%
  ...
  p_fiche_niveau <- reactive({
    ...
    ggplot(d, aes(degre, eff, fill = Sexe)) +
      ...
      geom_text(aes(label = paste0(fmt_eff(eff), " (", percent(part, accuracy = 0.1), ")")),
                position = position_dodge(width = 0.86), hjust = -0.05,
                size = 4.3) +
      coord_flip() +
  ...
  p_fiche_comp <- reactive({
    ...
    # Affichage du pourcentage blanc : un segment le porte dès que SA PROPRE
    # largeur suffit à contenir le texte. On raisonne en LARGEUR de segment :
    #   - le cadre de tracé fait environ « largeur_image moins colonne des noms »
    #     pouces de large et représente l'étendue [0 ; max(barre)] ;
    #   - la largeur d'un segment vaut donc (n / etendue) x largeur_cadre ;
    #   - « 54 % » fait 3 caractères, soit ~0,072 po/caractère à la taille 3,9,
    #     plus une petite marge intérieure.
    # Le bug précédent comparait l'effectif du segment à un seuil calibré sur la
    # PLUS GRANDE barre : un segment large porté par une petite composante était
    # alors rejeté à tort.
    TAILLE_PC     <- 3.9
    etendue       <- max(tot_comp$eff)
    l_cadre_est   <- max(1.5, 6.6 * (1 - marge / (1 + marge)) - 0.4)
    largeur_texte <- 3 * 0.072 * (TAILLE_PC / 3.5) + 0.10           # pouces
    # Largeur, en pouces, du segment de chaque sexe (colonne calculée en amont).
    d <- d %>% mutate(larg_seg = n / etendue * l_cadre_est)

    ggplot(d, aes(panneau, n, fill = Sexe)) +
      geom_col(width = 0.75) +
      geom_text(aes(label = ifelse(larg_seg >= largeur_texte & !is.na(part_sexe),
                                   percent(part_sexe, accuracy = 1), "")),
                position = position_stack(vjust = 0.5),
                colour = "white", fontface = "bold", size = TAILLE_PC) +
  ...
  # ---------------------------------------------------------------------------
  #  Présente en TABLEAUX les mêmes données que l'onglet Graphiques, pour la
  #  rentrée sélectionnée. Chaque tableau réutilise EXACTEMENT l'agrégation du
  #  graphique correspondant (même group_by, même source filtrée) afin que les
  ...
  # ---- Bandeau de chiffres clés (dont le TOTAL demandé) ---------------------
  output$synth_kpi <- renderUI({
    d <- data_filtree()
    tot <- sum(d$effectifs, na.rm = TRUE)
    f   <- sum(d$femmes,    na.rm = TRUE)
    h   <- sum(d$hommes,    na.rm = TRUE)
    boite <- function(titre, valeur, couleur = "#003F7D", larg = 2) {
      column(larg, tags$div(
        style = paste0("border-left:4px solid ", couleur,
                       "; background:#F7F9FC; padding:8px 10px; margin-bottom:6px;"),
        tags$div(style = "font-size:11px; color:#555;", titre),
        tags$div(style = paste0("font-size:19px; font-weight:700; color:", couleur, ";"),
  ...
  output$export_fiche <- downloadHandler(
    ...
    content = function(file) {
      ...
      # Nombre d'années réellement présentes dans le graphique d'évolution.
      # Au-delà de 6, le graphique (Total + Femmes + Hommes, soit 3 séries) est
      # trop dense en portrait : on le bascule sur une page PAYSAGE, où la
      # largeur supplémentaire aère les étiquettes.
      n_annees_evol <- length(unique(
        (fiche_all() %>% filtre_periode(input$per_fiche))$rentree))
      evol_paysage  <- n_annees_evol > 6
      i_evol <- if (evol_paysage) img(p_fiche_evol(), 9.6, 4.6)
                else               img(p_fiche_evol(), 6.6, 3.4)
      ...
      tp <- function(t, sz = 11, bold = FALSE, col = "black", it = FALSE)
        ftext(t, fp_text(font.family = "Arial", font.size = sz, bold = bold,
                         color = col, italic = it))
      # keep_with_next = TRUE : Word garde le titre sur la même page que le bloc
      # (tableau ou image) qui le suit immédiatement. Supprime les sauts de page
      # laissant un titre orphelin en bas de page.
      titre <- function(doc, t) body_add_fpar(doc, fpar(
        tp(t, 13, TRUE, "#003F7D"),
        fp_p = fp_par(padding.top = 12, padding.bottom = 4,
                      keep_with_next = TRUE)))
      ...
      # Si peu d'années, dans le flux portrait normal. Sinon, sur une page
      # paysage dédiée : on clôt d'abord la section portrait courante, on insère
      # le graphique large, puis on rouvre une section portrait pour la suite.
      if (!evol_paysage) {
        doc <- titre(doc, "Évolution des effectifs")
        doc <- body_add_img(doc, i_evol, width = 6.4, height = 3.3)
      } else {
        # body_end_block_section applique les propriétés passées à la section qui
        # PRÉCÈDE l'appel. On clôt donc d'abord tout le début du document en
        # PORTRAIT (nouvelle page), puis on isole le graphique dans sa propre
        # section PAYSAGE, close à son tour, avant que la suite reparte en
        # portrait via la section finale du document.
        doc <- body_end_block_section(doc, block_section(prop_section(
          page_size = page_size(orient = "portrait"), type = "nextPage")))
        doc <- titre(doc, "Évolution des effectifs")
        doc <- body_add_img(doc, i_evol, width = 9.4,
                            height = round(9.4 / 9.6 * 4.6, 2))
        doc <- body_end_block_section(doc, block_section(prop_section(
  ...
  # ===========================================================================
  # Liste ordonnée des établissements comparés (au moins deux requis).
  comp_choix <- reactive({
    e <- input$comp_etabs
    validate(need(length(e) >= 2,
                  "Sélectionnez au moins deux établissements à comparer."))
    e
  })
  comp_data <- reactive({
    e <- comp_choix()
    etab_aggreg %>%
      filter(etab_affiche %in% e) %>%
      mutate(Etab = factor(etab_affiche, levels = e))
  })
  # Palette propre aux établissements comparés : construite pour un nombre
  # QUELCONQUE d'établissements, et volontairement distincte du rouge/bleu de la
  # charte (réservés aux sexes et secteurs). En deçà de 8, une palette
  # qualitative lisible ; au-delà, on complète par interpolation.
  # Légende des établissements : repliée et empilée verticalement pour rester
  # lisible quel que soit le nombre d'établissements. Les noms sont longs ; sur
  # une seule ligne ils débordaient ou se tronquaient dès trois établissements.
  # Légende empilée : au-delà de 4 établissements on répartit sur 2 colonnes,
  # sinon une colonne unique (plus lisible). L'espacement vertical est augmenté
  # pour que deux noms repliés ne se touchent pas.
  legende_comp <- function(n) {
    nc <- if (n > 4) 2 else 1
  ...
  theme_legende_comp <- function() {
    ggplot2::theme(
      legend.position   = "top",
      legend.direction  = "vertical",
      legend.text       = ggplot2::element_text(size = 11.5, lineheight = 1.0),
      legend.title      = ggplot2::element_blank(),
      legend.key.size   = grid::unit(1.3, "lines"),
      legend.spacing.y  = grid::unit(0.4, "lines"),
      legend.margin     = ggplot2::margin(4, 4, 8, 4))
  }
  # Noms d'établissements repliés. Largeur plus généreuse (44) : la plupart des
  # noms tiennent alors sur une seule ligne, ce qui aère la légende.
  etiq_etab <- function(x, largeur = 44)
    vapply(as.character(x),
           function(s) paste(strwrap(s, width = largeur), collapse = "\n"),
           character(1), USE.NAMES = FALSE)
  ...
  col_comp <- reactive({
    e <- comp_choix()
    base <- c("#00807A", "#E07B00", "#6A3D9A", "#1F78B4", "#C7102C",
              "#33A02C", "#B15928", "#555555")
    cols <- if (length(e) <= length(base)) base[seq_along(e)]
            else grDevices::colorRampPalette(base)(length(e))
    setNames(cols, e)
  })
  ...
  comp_resume <- reactive({
    ...
    # Une colonne par établissement comparé, dans l'ordre de sélection.
    tables <- lapply(comp_choix(), resume)
    Reduce(function(acc, x) full_join(acc, x, by = "Indicateur"), tables)
  })
  ...
  p_comp_evol <- reactive({
    ...
    ggplot(d, aes(rentree, eff, colour = Etab)) +
      ...
      scale_colour_manual(values = col_comp(),
                          labels = function(x) etiq_etab(x, 44)) +
      scale_x_continuous(breaks = sort(unique(d$rentree))) +
      scale_y_continuous(labels = fmt_eff, breaks = breaks_effectifs(),
                         expand = expansion(mult = c(0.26, 0.22))) +
  ...
  p_comp_sexe <- reactive({
    d <- comp_data() %>% filter(rentree == input$rentree) %>%
      ...
      group_by(Etab) %>% mutate(part = eff / sum(eff)) %>% ungroup()
    validate(need(nrow(d) > 0, "Aucune donnée pour cette rentrée."))
    # Un panneau par établissement : Femmes / Hommes côte à côte à l'intérieur
    # de chaque panneau. Contrairement à des noms d'établissements alignés en
    # abscisse — qui se chevauchaient dès trois établissements —, les facettes
    # restent lisibles quel qu'en soit le nombre, le titre de chaque panneau
    # portant le nom replié.
    d <- d %>% mutate(Etab_lab = factor(etiq_etab(Etab, 26),
                                        levels = etiq_etab(levels(Etab), 26)))
    n <- nlevels(d$Etab)
    ggplot(d, aes(Sexe, eff, fill = Sexe)) +
      geom_col(width = 0.7) +
      geom_text(aes(label = paste0(fmt_eff(eff), "\n(",
                                   percent(part, accuracy = 0.1), ")")),
                vjust = -0.3, size = 3.5, lineheight = 0.9) +
      facet_wrap(~ Etab_lab, nrow = 1) +
      scale_fill_manual(values = col_sexe) +
      scale_y_continuous(labels = fmt_eff, expand = expansion(mult = c(0, 0.26))) +
      labs(title = NULL, subtitle = paste0("Rentrée ", input$rentree),
           x = NULL, y = "Étudiants inscrits", fill = NULL) +
      theme_obs +
  ...
  p_comp_tableau <- reactive({
    t <- comp_resume()
    nc <- ncol(t)                         # 1 colonne d'indicateurs + N établissements
    cols <- names(t)
    # Noms d'établissements repliés pour tenir dans l'en-tête quel qu'en soit
    # le nombre ; la taille du texte décroît quand les colonnes se resserrent.
    ent_lab <- c("Indicateur",
                 vapply(cols[-1],
                        function(x) paste(strwrap(x, width = max(12, 34 %/% (nc - 1))),
                                          collapse = "\n"),
                        character(1)))
    d <- tibble(
      ligne = rep(seq_len(nrow(t)), nc),
      col   = rep(seq_len(nc), each = nrow(t)),
      texte = unlist(lapply(t, as.character), use.names = FALSE)
    )
    ent <- tibble(col = seq_len(nc), texte = ent_lab)
    taille <- max(2.6, 4 - 0.18 * max(0, nc - 3))
    ggplot() +
      geom_rect(data = ent, aes(xmin = col - 0.5, xmax = col + 0.5,
                                ymin = 0.2, ymax = 1.6),
                fill = "#003F7D") +
      geom_text(data = ent, aes(col, 0.9, label = texte),
                colour = "white", fontface = "bold", size = taille, lineheight = 0.85) +
      ...
      geom_text(data = d, aes(col, ligne + 1.4, label = texte,
                ...
                size = taille, colour = "#222222") +
      scale_y_reverse() +
      scale_x_continuous(limits = c(0.5, nc + 0.5)) +
  ...
  # ---- Export de la synthèse régionale en Word ------------------------------
  #  Même logique graphique que l'export de la fiche établissement : en-tête
  #  (logo + mentions + titre), tableau de chiffres clés, puis les graphiques de
  #  l'onglet Graphiques insérés en images. Les graphiques larges ou hauts
  #  passent sur une page paysage pour rester lisibles.
  output$export_synthese <- downloadHandler(
    filename = function()
      paste0("Synthese_effectifs_Normandie_", input$rentree, ".docx"),
    content = function(file) {
      d <- data_filtree()
      validate(need(nrow(d) > 0, MSG_VIDE))
      tot <- sum(d$effectifs, na.rm = TRUE)
      ...
      # Images des graphiques (fichiers temporaires). On réutilise EXACTEMENT
      # les graphiques de l'onglet Graphiques, garantissant que le document
      # montre les mêmes visuels que l'application.
      img <- function(p, l, ht) {
        tmp <- tempfile(fileext = ".png")
        ggsave(tmp, plot = p, width = l, height = ht, dpi = 150, bg = "white")
        tmp
      }
      # Graphiques compacts (portrait) et graphiques hauts (paysage pleine page).
      i_niveau      <- img(p_niveau(),      6.6, 4.2)
      i_sexe        <- img(p_sexe(),        6.6, 4.6)
      i_secteur     <- img(p_secteur(),     6.6, 3.4)
      i_cat         <- img(p_cat(),         9.4, 5.6)   # paysage
      i_cat_sexe    <- img(p_cat_sexe(),    9.4, 6.4)   # paysage
      i_cat_secteur <- img(p_cat_secteur(), 9.4, 6.4)   # paysage
      n_uu          <- n_distinct(data_filtree()$unite_urbaine)
      h_uu          <- min(9.0, max(4.0, 0.32 * max(1, n_uu) + 1.6))
      i_uu          <- img(p_uu(),          9.4, h_uu)  # paysage, hauteur adaptée

      # Tableau de chiffres clés (mêmes valeurs que le bandeau de l'onglet).
      t_cles <- data.frame(
        Indicateur = c("Total étudiants inscrits", "dont femmes", "dont hommes",
                       "Établissements", "Communes d'implantation", "Catégories"),
        Valeur = c(fmt_eff(tot),
                   paste0(fmt_eff(f), " (", percent(f / tot, accuracy = 0.1), ")"),
                   paste0(fmt_eff(h), " (", percent(h / tot, accuracy = 0.1), ")"),
                   as.character(n_distinct(d$uai_etab)),
                   as.character(n_distinct(d$commune)),
                   as.character(n_distinct(d$categorie))),
        stringsAsFactors = FALSE, check.names = FALSE
      )
      ...
      doc <- body_add_fpar(doc, fpar(
        tp("Synthèse régionale des effectifs étudiants en Normandie",
           18, TRUE, "#003F7D"),
        fp_p = fp_par(padding.top = 10)))
      doc <- body_add_fpar(doc, fpar(
          tp(paste0("Grands chiffres — rentrée ", input$rentree),
             11, it = TRUE, col = "#555555"),
          fp_p = fp_par(border.bottom = fp_border(color = "#C7102C", width = 2),
                        padding.bottom = 6)))
      ...
      # ---- Graphiques compacts (portrait) -----------------------------------
      doc <- titre(doc, "Répartition par niveau d'études")
      doc <- body_add_img(doc, i_niveau, width = 6.4, height = round(6.4/6.6*4.2, 2))

      doc <- titre(doc, "Répartition par sexe et par niveau d'études")
      doc <- body_add_img(doc, i_sexe, width = 6.4, height = round(6.4/6.6*4.6, 2))

      doc <- titre(doc, "Répartition par secteur")
      doc <- body_add_img(doc, i_secteur, width = 6.4, height = round(6.4/6.6*3.4, 2))

      # ---- Graphiques larges / hauts (page paysage dédiée) ------------------
      # On clôt la section portrait, on insère les graphiques larges en paysage,
      # puis on rouvre une section portrait pour le pied de page.
      doc <- body_end_block_section(doc, block_section(prop_section(
        page_size = page_size(orient = "portrait"), type = "nextPage")))

      doc <- titre(doc, "Répartition par catégorie d'établissement")
      doc <- body_add_img(doc, i_cat, width = 9.4, height = round(9.4/9.4*5.6, 2))

      doc <- titre(doc, "Répartition par catégorie et par sexe")
      doc <- body_add_img(doc, i_cat_sexe, width = 9.4, height = round(9.4/9.4*6.4, 2))

      doc <- titre(doc, "Répartition par catégorie et par secteur")
      doc <- body_add_img(doc, i_cat_secteur, width = 9.4, height = round(9.4/9.4*6.4, 2))

      doc <- titre(doc, "Répartition par unité urbaine")
      doc <- body_add_img(doc, i_uu, width = 9.4, height = round(9.4/9.4*h_uu, 2))
      ...
      print(doc, target = file)
      unlink(c(i_niveau, i_sexe, i_secteur, i_cat, i_cat_sexe, i_cat_secteur, i_uu))
    }
  )
  ...
  output$export_comparateur <- downloadHandler(
    filename = function() {
      ...
      e <- comp_choix()
      etiquette <- if (length(e) <= 2) paste(vapply(e, net, character(1)),
                                             collapse = "_vs_")
                   else paste0(length(e), "_etablissements")
      paste0("Comparateur_", etiquette, "_", input$rentree, ".pdf")
    },
    content = function(file) {
      ...
      tryCatch({
        ...
        print(graphique_message(paste(
          ...
          "Sélectionnez au moins deux établissements",
  ...
  output$tableau_etab <- renderDT({
    vue <- if (is.null(input$vue_synthese)) "commune" else input$vue_synthese

    if (vue == "commune") {
      # --- Agrégation par COMMUNE : une ligne, effectifs sommés ------------
      # Répond au besoin « somme automatique des effectifs sur la commune ».
      # Le nombre d'établissements et la ventilation par sexe accompagnent le
      # total pour donner du contexte sans quitter la vue.
      d <- data_filtree() %>%
        group_by(Commune = commune) %>%
        summarise(`Établissements` = n_distinct(uai_etab),
                  Effectifs = sum(effectifs, na.rm = TRUE),
                  Femmes    = sum(femmes,    na.rm = TRUE),
                  Hommes    = sum(hommes,    na.rm = TRUE),
                  .groups = "drop") %>%
        arrange(desc(Effectifs)) %>%
        mutate(across(c(Effectifs, Femmes, Hommes, `Établissements`), fmt_eff))
      return(datatable(
        d, filter = "top",
        ...
        caption = htmltools::tags$caption(
          style = "caption-side:top; font-weight:700; color:#003F7D;",
          "Effectifs cumulés par commune — sélection courante")))
    }

    # --- Détail par ÉTABLISSEMENT (vue historique) -----------------------
  ...
  output$export_map <- downloadHandler(
    filename = function() paste0("Carte_Etudiants_Normandie_", input$rentree, "_",
                                 Sys.Date(), ".html"),
    content = function(file) {
      # selfcontained = TRUE exige pandoc, fourni par RStudio sur le poste de
      # travail mais ABSENT de la version navigateur (Shinylive) : R y est
      # compilé en WebAssembly, sans outil externe.
      # L'ancien repli en fichier non autoportant produisait un HTML renvoyant
      # à un dossier de ressources que le téléchargement ne transmet pas —
      # donc une page blanche, plus déroutante qu'un refus explicite. On
      # préfère désormais un message clair.
      tryCatch({
        m <- construire_carte(points_filtree(), cantons_filtree(), arr_filtree(),
                              opts_courantes(), vue_courante())
        tryCatch(saveWidget(m, file, selfcontained = TRUE), error = function(e) {
          writeLines(paste0(
            "<html><meta charset='utf-8'><body style=\"font-family:Arial\">",
            "<h3 style='color:#003F7D'>Export de carte indisponible</h3>",
            "<p>L'export d'une carte autonome nécessite un composant absent de ",
            "la version consultée dans le navigateur.</p>",
            "<p>Les exports Word, PDF et PNG restent disponibles ; ",
            "l'export de la carte fonctionne dans la version installée sur ",
            "poste de travail.</p></body></html>"), file)
        })
      }, error = function(e) {
        writeLines(paste0(
          "<html><meta charset='utf-8'><body style=\"font-family:Arial\">",
